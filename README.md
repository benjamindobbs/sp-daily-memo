...
|---|---|---|
| Master Schedule | `/master-schedule` | All classes by period. Search bar filters by class name or camper name. Filter by period, side, and color group. Click a class name to open its roster. Instructor, Unit Leader, and Sports Leader names are clickable links to their staff profile for all users. |
...
|---|---|---|
| Counselor Preferences | `/counselor-preferences` | View and edit each counselor's activity preferences, used by the auto-scheduler to influence assignments. The activity list reflects next week's offerings when they've been uploaded (falling back to the current week), and the form notes which week is shown |
