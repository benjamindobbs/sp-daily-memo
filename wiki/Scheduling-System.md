...
3. Each card has a **"Skip this period"** checkbox (no coverage needed that period) and a covering-counselor `<select>`, built by `buildCoverageCandidates(period, side, week, excludeCounselorId)` plus, for leadership/instructor roles, `buildLeadershipCandidates(role, period, week, excludeCounselorId, requireScheduledThisWeek)`:
   ...
   **Same-day override:** a candidate's "current assignment" isn't always their static `CounselorWeekSchedules`/`StaffWeekSchedules` row — `buildCoverageOverridesByCounselor(coverageMap)` re-keys the date's coverage map by `CoveringCounselorID|PeriodNumber`, so if the candidate has already been pulled to cover a *different* out-counselor's class this same period earlier in the session, their annotation (activity, headcount, enrollment, and the ✅/⚠️ tags below) reflects the class they're now actually covering, not their original schedule.

   **✅ Extra tag:** a candidate is tagged "Extra" (`isExtra`, `computeIsExtra()`) when pulling them off their current class would still leave it at or under a 1:12 camper:counselor ratio (`currentEnrollment / (currentHeadcount - 1) <= 12`) — or when they have no current assignment at all (nothing to protect). A candidate who is the sole counselor on their class is never tagged Extra, regardless of enrollment. Applies to every bucket, including Unit Leaders/Sports Leaders/Instructors.

   **⚠️ Already Covering tag:** a candidate already appears as the `CoveringCounselorID` on some other `CounselorCoverage` row for this exact date+period (`isAlreadyCovering`) is highlighted red (`.already-covering-option`) and text-tagged, so admins don't double-book them. Not a hard block — informational only, matching the "never filtered out" convention above. Takes visual priority over the green Send to Sports highlight when both apply to the same swim-counselor candidate.
...
4. **Save** (`POST /coverage/save`) upserts one `CounselorCoverage` row per period (parallel arrays, same convention as the swim-scheduling guard checklists); a period with neither a covering counselor nor Skip checked has its row cleared instead. Re-visiting the same date/counselor pre-fills every card from the existing rows.
   ...
   **Coverage alert:** for each period where a covering counselor is newly assigned or changed from what was previously saved (compared against the prior `CounselorCoverage` row for that `Date`/`OutCounselorID`/`PeriodNumber` before the upsert), a push notification is sent to that covering counselor via the existing Instant Alerts infrastructure (`resolveAlertTargets('individual', ...)` + `sendInstantAlert()` — see [Auth](./Auth.md) or the Instant Alerts wiki page) naming the period and the class they're now covering. Re-saving a form with no changes, and clearing/skipping a period, never sends an alert.
...
- **Attendance overview, staff self-filtered view** (`/attendance` with a `selectedCounselor` cookie set and `showAll` not `1`) — `allowedClasses` (the set of `period|activityName` the filter shows) additionally includes any class the covering counselor is covering that date, alongside their real `CounselorWeekSchedules`/`StaffWeekSchedules`/`CounselorScheduleAssignments`-sourced classes, so the covered class isn't invisible to them when checking their own day. The flat session list then annotates each class card:
  ...
  - A class the viewer is out of, with a substitute actually assigned (they're the `OutCounselorID` on a `CounselorCoverage` row with `Skipped=0` and a `CoveringCounselorID`), is greyed out (`att-flat-covered-away`) and tagged "Covered by X". A `Skipped=1` row ("no coverage needed") does *not* grey the card out — nobody else is taking the class, so it stays a fully normal, actionable card for the original counselor rather than looking handled.
