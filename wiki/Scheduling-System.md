...
3. Each card has a **"Skip this period"** checkbox (no coverage needed that period) and a covering-counselor `<select>`, built by `buildCoverageCandidates(period, side, week, excludeCounselorId)` plus, for leadership/instructor roles, `buildLeadershipCandidates(role, period, week, excludeCounselorId, requireScheduledThisWeek)`:
   ...
   Every remaining candidate (in any bucket) is annotated with **their own current assignment that period** — activity name, how many counselors are already on it, and its enrollment — so the admin can judge the cost of pulling them off it. The out counselor themself is excluded from every bucket. The Unit Leaders/Sports Leaders/Instructors buckets are additive — covering one of those roles still also shows the normal correct-side/swim/opposite-side buckets after them, unchanged. Unit Leaders+Sports Leaders and Instructors are mutually exclusive (keyed off the out person's own `StaffRole`), so a card only ever shows one of the two special-role bucket sets.

   **Already-covering exclusion:** a candidate already covering a *different* out-counselor's class this exact date+period is removed from every bucket entirely, not just flagged — they can't be double-booked into a second assignment for the same period. `buildCoverageExclusions(coverageMap, excludeOutCounselorId)` builds a `CoveringCounselorID|PeriodNumber` set from the date's coverage map, excluding rows that belong to `excludeOutCounselorId` (the out-counselor whose card is currently being built) — so a counselor already saved as the covering pick for *this* card's own period stays available (their existing selection is still visible/editable on a re-visit), while a pick already committed to someone else's period is filtered out before the buckets are even split. Both `buildCoverageCandidates()` and `buildLeadershipCandidates()` take this set and filter their pool up front.
